...
void update_screen_values(lv_ui *ui)
{
	...
	static uint8_t previous_window			  = 3;
	uint16_t degrees_other                    = 0;
	uint16_t degrees_idle                     = 0;
	uint16_t degrees_lgvl                     = 0;
	uint16_t degrees_server                   = 0;

	uint8_t actual_window = get_actual_window();
	if(previous_window != actual_window)
	{
		previous_window = actual_window;
		switch(previous_window)
			{
				case 1:
...
void setup_scr_screen(lv_ui *ui)
{
	...
	lv_arc_set_angles(ui->screen_CounterArc, 0, 0);
